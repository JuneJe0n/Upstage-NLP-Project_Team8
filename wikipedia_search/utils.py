...
def get_choices(prompts):
    choices = []
    for prompt in prompts:
        # 첫 번째 줄바꿈을 기준으로 질문과 선택지를 분리
        lines = prompt.split('\n', 1)  # 첫 번째 줄바꿈만 기준으로 분리
        
        # 첫 번째 줄바꿈 이전은 질문, 이후는 선택지로 간주
...
def extract_question_keywords(prompts):
    # Upstage API 설정
    UPSTAGE_API_KEY = "up_0L7RvF2YEPh96TxA3du1a1W225uIg"
    llm = ChatUpstage(api_key=UPSTAGE_API_KEY, model="solar-1-mini-chat")
    ...
    prompt_template = PromptTemplate.from_template(
        ...
        1. Identify the problem type (e.g., "Math Problem", "General Knowledge", "Legal Question", etc.).
    ...
    for prompt in prompts:
        input_dict = {"question_text": prompt}
        response = chain.invoke(input_dict).content.strip()  # 응답에서 공백 제거
        try:
            # 응답을 딕셔너리로 변환
