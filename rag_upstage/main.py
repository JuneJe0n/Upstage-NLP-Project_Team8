...
from util import (read_test_data, split_documents, check_question,detect_missing_context,
                  extract_question_queries, extract_question_keywords, fetch_wiki_page, sem_split_documents, accuracy)
from generate_prompt import classify_mmlu_domain, generate_prompt
import uuid
...
def main():
    ...
    for i, prompt in enumerate(prompts):
        response = query_rag(prompt,ewha_collection,wiki_collection, f"QUESTION{i+1}")
...
def query_rag(prompt: str, ewha_collection, wiki_collection, question_number: str):
    # Load chat model
    ...
    ################# If context doesn't exist in ewha policy pdf, search wiki#################
        ...
        try:
            ...
            results_wiki = post_process_results(results_wiki, similarity_threshold=SIMILARITY_THRESHOLD)
        ...
        prompt_template_wiki = generate_prompt(
            problem_type=problem_type, domain=domain)
        prompt = PromptTemplate.from_template(prompt_template_wiki)
